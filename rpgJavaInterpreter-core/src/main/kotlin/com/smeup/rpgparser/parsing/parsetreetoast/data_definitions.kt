                ...
                if (initializationValue != null) {
                    ...
                    when (type.format) {
                        DateFormat.JUL -> {
                            ...
                            minCal.set(1940, Calendar.JANUARY, 1, 0, 0, 0)
                            minCal.set(Calendar.MILLISECOND, 0)
                            val maxCal = Calendar.getInstance()
                            maxCal.set(2039, Calendar.DECEMBER, 31, 23, 59, 59)
                            maxCal.set(Calendar.MILLISECOND, 999)
                            if (!dateInz.after(minCal.time) || !dateInz.before(maxCal.time)) {
                                error(message = "For JUL format the date must be between 1940 and 2039", conf = conf)
                            }
                        }
                        DateFormat.ISO -> {
                            val minCal = Calendar.getInstance()
                            minCal.set(1, Calendar.JANUARY, 1, 0, 0, 0)
                            minCal.set(Calendar.MILLISECOND, 0)
                            val maxCal = Calendar.getInstance()
                            maxCal.set(9999, Calendar.DECEMBER, 31, 23, 59, 59)
                            maxCal.set(Calendar.MILLISECOND, 999)
                            if (!dateInz.after(minCal.time) || !dateInz.before(maxCal.time)) {
